...
const setup = async () => {
  ...
  // click add recipe
  userEvent.click( app.getByRole('button', {name: 'Add Recipe'}) );

  let instructionsInput = await app.findByRole('textbox', {name: /instructions/i});
  // or potentially...
  // let instructionsInput = await app.findByLabelText('Instructions:')
...
test('typing in the recipe instructions makes the instructions appear in the form', async () => {
  const {app, instructionsInput} = await setup();

  const recipeInstructions = "kinda hard to write instructions without knowing what I'm cooking";
  const recipeName = "Test Recipe";

  userEvent.type(app.getByLabelText(/Recipe name/i), recipeName);
  userEvent.type(instructionsInput, recipeInstructions);
  expect(instructionsInput.value).toEqual(recipeInstructions);
  userEvent.click(app.getByRole('button', { name: /submit/i }));

  const recipeTitle = await app.findByText(`Name: ${recipeName}`);
  userEvent.click(recipeTitle);

  await app.findByText(`Instructions: ${recipeInstructions}`);
});

test('clicking a recipe title shows its instructions', async () => {
  render(<App />);

 
  userEvent.click(screen.getByRole('button', { name: 'Add Recipe' }));
  await screen.findByRole('form');
  userEvent.type(screen.getByLabelText(/Recipe name/i), 'Pizza');
  userEvent.type(screen.getByLabelText(/Instructions/i), 'Cook pizza');
  userEvent.click(screen.getByRole('button', { name: /submit/i }));

 
  userEvent.click(screen.getByRole('button', { name: 'Add Recipe' }));
  await screen.findByRole('form');
  userEvent.type(screen.getByLabelText(/Recipe name/i), 'Salad');
  userEvent.type(screen.getByLabelText(/Instructions/i), 'Toss salad');
  userEvent.click(screen.getByRole('button', { name: /submit/i }));

 
  expect(screen.getByText(/Name: Pizza/i)).toBeInTheDocument();
  expect(screen.getByText(/Name: Salad/i)).toBeInTheDocument();


  expect(screen.queryByText(/Instructions: Cook pizza/i)).not.toBeInTheDocument();
  expect(screen.queryByText(/Instructions: Toss salad/i)).not.toBeInTheDocument();

  
  userEvent.click(screen.getByText(/Name: Pizza/i));
  expect(screen.getByText(/Instructions: Cook pizza/i)).toBeInTheDocument();

  
  expect(screen.queryByText(/Instructions: Toss salad/i)).not.toBeInTheDocument();
  userEvent.click(screen.getByText(/Name: Salad/i));

  
  expect(screen.getByText(/Instructions: Toss salad/i)).toBeInTheDocument();
  expect(screen.queryByText(/Instructions: Cook pizza/i)).not.toBeInTheDocument();
});